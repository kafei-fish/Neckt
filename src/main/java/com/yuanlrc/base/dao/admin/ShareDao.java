...
public interface ShareDao extends JpaRepository<Share, Long>,JpaSpecificationExecutor<Share> {
	...
	@Query(value="select count(s.id) as num,DATE_FORMAT(create_time,'%Y-%m-%d') as shareDate from ylrc_share s WHERE s.user_id = :userId and s.share_type = :shareType GROUP BY DATE_FORMAT(create_time,'%Y%m%d') order by DATE_FORMAT(create_time,'%Y%m%d') asc",nativeQuery=true)
